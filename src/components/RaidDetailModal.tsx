...
import { generateFullRanking, fmtDate, type RankedEntry } from '@/lib/utils'
...
export function RaidDetailModal({ raid, onClose, onPlayerClick }: Props) {
  const [full, setFull] = useState<RankedEntry[]>([])

  useEffect(() => {
    fetch(`/api/raids/${raid.id}/entries`)
      .then((r) => r.json())
      .then((data: RankedEntry[]) => setFull(generateFullRanking(data)))
  }, [raid.id])

  return (
    <StModal
      title={`${raid.name} — ${raid.episode} · Full Rankings (Top 50)`}
      onClose={onClose}
      wide
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 12, color: 'var(--muted)' }}>
            {raid.season} · {fmtDate(raid.startDate)} — {fmtDate(raid.endDate)}
          </span>
        </div>
      ...
      <div style={{ maxHeight: '62vh', overflowY: 'auto', borderRadius: 10, border: '1px solid var(--border)' }}>
